...
public class MobileService {
	...
	public List<Mobile> year(int a) throws YearException{
//		List<Mobile> m= new ArrayList<>();
//		List<Mobile> collect = m.stream().filter(x->x.getYear()>a).collect(Collectors.toList());
//		if(collect.isEmpty())
//			throw new YearException("Mobile year not found Exception");
//		else
//			return md.year(a);
		List<Mobile> mobiles = md.year(a);
		if (mobiles.isEmpty())   
	        throw new YearException("Mobile year not found Exception");
	    else
	        return mobiles;
